...
This code implements the A-Star algorithm to minimize the total distance traveled by the robot
"""
import IPython
import math
import sys
...
def get_cost(C,state):
	...
	This function returns a large cost if the pixel value is > 200 (white);
	Else it returns 1 as the cost"""
	(x,y) = state
	if im[x][y] > 200:
		return 1000000
	else:
...
def get_heuristic(state1, state2):
	...
	(x1,y1) = state1
	(x2,y2) = state2
	return 1*math.fabs((x1-x2) + (y1-y2))
...
def get_successors(state):
	"""Input: State 
   	...
   	This function assumes a four connected gridworld and outputs the successors of the 
	current state"""
	(x,y) = state
